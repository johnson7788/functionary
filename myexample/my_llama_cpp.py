...
class FunctionaryAPI:
    def __init__(self):
        ...
        model_repo = "meetkai/functionary-small-v2.4-GGUF"

        # File to download, 可选functionary-small-v2.4.Q4_0.gguf，functionary-small-v2.4.Q8_0.gguf，functionary-small-v2.4.f16.gguf
        file_name = "functionary-small-v2.4.f16.gguf"

        # Download the file
        # local_file_path = hf_hub_download(repo_id=model_repo, filename=file_name)
        local_file_path = os.path.join(model_repo,file_name)
...
async def main():
    ...
    if response.get("tool_calls") is not None:
        ...
        for tool in response["tool_calls"]:
            ...
            tool_call_response = tool_result(tool["id"], content=str(result), name=requested_function["name"])
